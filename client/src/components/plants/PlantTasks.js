...
function Tasks(props) {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/plants/tasks/' + id)
            .then(res => {
...
console.log(plant.plantAge)
    ...
    const changehandle = (taskId)=>{
        ...
        .then(res => {
            ...
            axios.put('http://localhost:8000/api/tasks/update/' + taskId, currntTask)
                .then(res => {
                    console.log(res);
                ...
                <div className={css.divCenter}>
                    {tasks.map(task => (
                        <div className={css.box}>
                            {task.taskName}
                            {task.data}
                            <input type="checkbox" value={task.isDone} checked={task.isDone} onClick={ (e ) => changehandle(task._id) }/>
                        </div>
